...
def get_polynomial_form(polynomial_degree: int) -> np.ndarray:
    ...
    """
    return np.array([[i] for i in range(0,int(polynomial_degree)+1)])
    ...
...
def print_polynomial(theta: np.ndarray, precission: int = 3) -> str:
    ...
    result=""
    for exponentIndex in range(0, len(theta)):
        result+=" + "+str(np.round_(theta[exponentIndex][0],precission))+"*x^"+str(exponentIndex)
...
def getCoefficients(X:np.ndarray, Y:np.ndarray):
    return np.transpose(np.array([np.matmul(np.matmul(np.linalg.inv(np.matmul(np.transpose(X),X)),np.transpose(X)),Y)]))
...
def least_squares_solution(
    ...
    X_pol = np.c_[np.ones(len(X))]
    

    for exp in range(1,polynomial_degree+1):
        X_pol=np.c_[X_pol,X**exp]

    result = getCoefficients(X_pol,Y)
...
def generalised_linear_model(X: np.ndarray, T: np.ndarray) -> np.ndarray:
    ...
    """
    result = sum([coeff * X ** degree for degree, coeff in enumerate(T)])
    return result
