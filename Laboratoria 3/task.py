...
def get_minor_matrix(mat: List[List[int]], i: int, j: int) -> List[List[int]]:
    ...
    if not is_matrix_square(mat) or len(mat)==0 or len(mat)<=i or len(mat[0])<=j:
        raise ValueError()
    else:
        resultList=[]
        ...
        for row in range(len(mat)):
            if(row!=i):
                ...
                for column in range(len(mat)):
                    if column!=j:
                        resultList[len(resultList)-1].append(mat[row][column])
...
def matrix_determinant(mat: List[List[int]]) -> int:
    ...
    if not is_matrix_square(mat):
        ...
        for column in range(len(mat)):
                print(mat)
                sum= sum+ mat[0][column]*matrix_determinant(get_minor_matrix(mat,0,column))*inc
                inc*=-1
